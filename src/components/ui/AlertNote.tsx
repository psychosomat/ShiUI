...
export function AlertNote({ className, children }: { className?: string; children?: React.ReactNode }) {
  ...
  return (
    <div role="alert" className={cn("flex gap-3 border-[1.5px] border-sumi bg-paper p-3.5", className)}>
      <span className="flex h-6 w-6 shrink-0 items-center justify-center border-[1.5px] border-sumi font-mono text-[12px] font-bold">i</span>
      <div><div className="font-sans text-[13px] font-bold">Proofs ready Thursday</div><div className="font-sans text-[13px] text-sumi/70">{children ?? "Two posters passed inspection. One needs a second pass of vermilion."}</div></div>
