...
export function CardInk({ className, title, meta, footer, children }: { className?: string; title?: string; meta?: string; footer?: React.ReactNode; children?: React.ReactNode }) {
  return (
    <div className={cn("border-[1.5px] border-sumi bg-paper", className)}>
      ...
      <div className="p-4 font-sans text-[13px] leading-relaxed">{children ?? "A generic ink container: any record, notice or preview lives here. Header carries the title, footer carries the action."}</div>
