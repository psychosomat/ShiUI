...
export function ButtonLarge({ className, children }: { className?: string; children?: React.ReactNode }) {
  return (
    <button
      type="button"
      className={cn(
        "group inline-flex w-full select-none items-center justify-between gap-2 border-[1.5px] border-sumi bg-shu font-sans font-bold uppercase cursor-pointer transition-colors duration-200 hover:bg-sumi active:translate-y-[1px]",
        "h-12 px-6 text-[14px] tracking-[0.14em] text-paper",
        className,
      )}
    >
